...
The goal of gpx_plotter is to plot GPS data from multiple .gpx files onto a map. Gpx files used below were sports activities during 2020 - 2021 in Uppsala. Output files from the script are:
...
map1 <- ggmap(maps[[1]]) + labs(title = names(maps[1]))
map2 <- ggmap(maps[[2]]) + labs(title = names(maps[2]))
map3 <- ggmap(maps[[3]]) + labs(title = names(maps[3]))


p_maps <- ggarrange(map1, map2, map3, nrow = 1)
ggsave(p_maps, path = "README_figures", filename = "readme_maps.png", device = "png", width = 36, height = 8)
